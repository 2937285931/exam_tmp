...
训练主脚本：数据加载、模型训练、评估、可视化、错误分析、结果汇总。
=============================================================================
"""
...
# 确保项目根目录在 sys.path 中，以支持从项目根目录执行 python src/train.py
...
from src.train_utils import EarlyStopping, train_epoch, evaluate, train_model
...
def plot_training_curves(histories: dict, config: dict):
    ...
    plt.tight_layout()
    save_path = os.path.join(config["saved_dir"], "loss_curve.png")
...
def analyze_lstm_errors(
    ...
    输出影评原文片段及模型预测置信度，写入 results/error_cases.txt。
...
def generate_results_summary(results: dict, errors_text: str, config: dict):
    """
    生成模型对比结果汇总文件 results/results.txt。
...
def main():
    ...
    # ---- 本地数据加载（不再使用 HuggingFace Datasets） ----
    ...
    cnn_model, cnn_history = train_model(
        ...
        save_best_path=os.path.join(cfg["saved_dir"], "cnn_model.pth"),
    )
    ...
    lstm_model, lstm_history = train_model(
        ...
        save_best_path=os.path.join(cfg["saved_dir"], "lstm_model.pth"),
    )
    ...
    print("  - loss_curve.png, error_cases.txt, results.txt")
